...
export function BotClient({ tasks, members, sprints, projectId }: Props) {
  // ── Shared ──────────────────────────────────────────────────────────────────
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            {chatMessages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-700">Hỏi PM Bot về dự án</p>
                  <p className="text-xs text-slate-400 mt-0.5">Phân tích tiến độ, task, sprint và hơn thế nữa</p>
                </div>
                <div className="grid grid-cols-1 gap-2 w-full max-w-sm">
                  {[
                    ...
                    <button key={q} onClick={() => { setChatInput(q) }}
                      className="text-xs text-left px-3 py-2 rounded-xl border border-slate-200 text-slate-600 hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700 transition-colors">
                      {q}
                    </button>
                  ))}
                </div>
